...
RED.search = (function() {
            ...
            for (i=0;i<keys.length;i++) {
                ...
                if (kpos > -1) {
                    ...
                    for (j=0;j<ids.length;j++) {
                        ...
                        var isConfigNode = false;// config nodes don't exists in this tool, node.node._def.category === "config" && node.node.type !== 'group';
        ...
        searchResults = $('<ol>',{style:"position: absolute;top: 5px;bottom: 5px;left: 5px;right: 5px;"}).appendTo(searchResultsDiv).editableList({
            ...
            addItem: function(container,i,object) {
                ...
                if (object.more) {
                    ...
                    div.text("palette.editor.more");//,{count:object.more.results.length-object.more.start}));
                    ...
                    $('<div>',{class:"red-ui-search-empty"}).text('search.empty').appendTo(container);
                } else {
                    var def = node._def;
    ...
    function toggle(args){
        if (visible == false) show(args);
        else hide();
    }

    function init() {
        RED.actions.add("core:search",toggle);
