...
RED.OCPview = (function() {
    ...
    var catLabel = "OCPview"; // so that this can be used in the "apply to other tabs" popuptext
    ...
    function init(chartId) {
        ...
        outer_background = vis.append('svg:rect').attr("class", "ws-chart-outer-background");

        gridScale = d3.scale.linear().range([0,1000]).domain([0,1000]);
        ...
        grid = vis.append('g').attr("class", "ws-chart-grid").attr("style", "visibility: visble;");

        // minors gets rendered first
        gridHminor = grid.append('g').attr({
            "id":chartId + "grid-h-mi",
            "style":"display:none;"
            });
        
        gridVminor = grid.append('g').attr({ 
            "id":chartId + "grid-v-mi",
            "style":"display:none;"
        });
        // major then gets rendered over the minors
        gridHmajor = grid.append('g').attr({
            "id":chartId + "grid-h-ma",
            "style":"display:none;"
            });
        gridVmajor = grid.append('g').attr({ 
            "id":chartId + "grid-v-ma",
            "style":"display:none;"
            });

        visGroups = vis.append('g').attr("class", "workspace-chart-groups");

        visLinks = vis.append('g').attr("class", "workspace-chart-links");

        drag_line = vis.append("svg:path").attr("class", "drag_line");

        visNodes = vis.append('g').attr("class", "workspace-chart-nodes");

        $("#"+chartId).on('DOMMouseScroll mousewheel', function (evt) {
        ...
        $("#"+chartId).droppable({
            ...
            drop: function( event, ui ) {
                console.error("node dropped into ocp");
                d3.event = event;
    ...
    function createTab(chartName)
	{
		var content = document.createElement("div");
		content.id = chartName;
		content.style.paddingTop = "4px";
		content.style.paddingLeft = "4px";
		content.style.paddingRight = "4px";
		RED.sidebar.addTab("OCP",content);
	...
	{
		var nn = {x: xPos,y:yPos,w:node_def.width,z:RED.view.getWorkspace()};
		...
		nn.id = RED.nodes.cppId(nn, RED.nodes.getWorkspace(RED.view.getWorkspace()).label);  // jannik add/change
	...
	function canvasTouchStart()
	{
		...
		if (d3.event.touches.length>1) {
			...
			var scrollPos = [$(chartId).scrollLeft(),$(chartId).scrollTop()];
	...
	function canvasTouchMove()
	{
		...
		if (d3.event.touches.length<2) {
			...
			if (!isNaN(moveTouchDistance)) {
				...
				$("#" + chartName).scrollLeft(scrollPos[0]+deltaTouchCenter[0]);
				$("#" + chartName).scrollTop(scrollPos[1]+deltaTouchCenter[1]);
				//redraw();
				//redraw_links_init();
				//redraw_links();
			}
